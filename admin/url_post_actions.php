...
if($mode == "pet_adoption")
{
	...
	$url=$database->escape_value ($_REQUEST["url"]);
    	$_SESSION["pet_adoptionEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
		$qry_update="INSERT INTO `".TBL_PET_ADOPTION."` (`title`,`url`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$title."','".$url."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
                  ...
                  if($result_upload >0)
	{
		...
		redirect_to('homelesspetadopt_manage.php');;
	}
	else
	{
		$error="Updating failed!.";
		redirect_to('homelesspetadopt_new.php');
	}
...
if($mode == "video_gallery")
{
	...
	$title=$database->escape_value ($_REQUEST["title"]);
	$url=$database->escape_value ($_REQUEST["url"]);
    	$_SESSION["video_galleryEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
		$qry_update="INSERT INTO `".TBL_VIDEO_GALLERY."` (`title`,`url`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$title."','".$url."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
			$result_upload = $database->query( $qry_update );
	...
	{
		$msg="Updated successfully!.";
		redirect_to('video_gallery_manage.php');;
	}
	else
	{
		$error="Updating failed!.";
		redirect_to('video_gallery_new.php');
	}
...
if($mode == "action_team")
{
	...
	$title=$database->escape_value ($_REQUEST["title"]);
	$content=$database->escape_value ($_REQUEST["content"]);
	$url=$database->escape_value ($_REQUEST["url"]);
    	$_SESSION["action_teamEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
		$qry_update="INSERT INTO `".TBL_ACTION_TEAM."` (`title`,`content`,`url`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$title."','".$content."','".$url."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
			$result_upload = $database->query( $qry_update );
			//echo "The image {$_FILES['media']['name'][$i]} was successfully uploaded and added to the gallery<br />";
	...
	{
		$msg="Updated successfully!.";
		redirect_to('action_team_manage.php');;
	}
	else
	{
		$error="Updating failed!.";
		redirect_to('action_team_new.php');
	}
