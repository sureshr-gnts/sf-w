...
if($mode == "flash_news")
{
	...
	$workwithus=$database->escape_value ($_REQUEST["workwithus"]);
	$positions=$database->escape_value ($_REQUEST["positions"]);
	$salary=$_REQUEST["salary"];
	$pleasenote=$_REQUEST["pleasenote"];

    	$_SESSION["flash_newsEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
			$qry_update="INSERT INTO `".TBL_FLASH_NEWS."` (`workwithus`,`positions`,`salary`,`pleasenote`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$workwithus."','".$positions."', '".$salary."','".$pleasenote."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
