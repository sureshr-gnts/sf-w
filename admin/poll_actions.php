...
if($mode == "upload")
{
	...
	$question=$database->escape_value ($_REQUEST["question"]);
	$answer_one=$database->escape_value ($_REQUEST["answer_one"]);
	$answer_two=$_REQUEST["answer_two"];
    $_SESSION["pollEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
			$qry_update="INSERT INTO `".TBL_POLL."` (`question`,`answer_one`,`answer_two`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$question."','".$answer_one."', '".$answer_two."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
                  ...
                  if($result_upload >0)
	{
		...
		redirect_to('poll_manage.php');;
	}
	else
	{
		$error="Updating failed!.";
		redirect_to('poll_new.php');
	}
