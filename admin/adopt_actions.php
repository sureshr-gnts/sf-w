...
if($mode == "adopt")
{
	...
	$lifepet_foster=$database->escape_value ($_REQUEST["lifepet_foster"]);
	$adoption_process=$database->escape_value ($_REQUEST["adoption_process"]);
	$meeting=$_REQUEST["meeting"];
	$application=$_REQUEST["application"];
	$screening=$_REQUEST["screening"];
	$home_visit=$_REQUEST["home_visit"];
    	$approval=$_REQUEST["approval"];
    	$followup=$_REQUEST["followup"];
    	$clinic=$_REQUEST["clinic"];
    	$medical_services=$_REQUEST["medical_services"];
    	$dogs_url=$_REQUEST["dogs_url"];
    	$cats_url=$_REQUEST["cats_url"];
    	$_SESSION["adoptEdit_formData"]=$_REQUEST;
                     
	

	        global $database, $db;
			$qry_update="INSERT INTO `".TBL_ADOPT."` (`lifepet_foster`,`adoption_process`,`meeting`,`application`,`screening`,`home_visit`,`approval`,`followup`,`clinic`,`medical_services`,`dogs_url`,`cats_url`,`created_from`,`created_by`,`created_dt`)"
                        . " VALUES ('".$lifepet_foster."','".$adoption_process."', '".$meeting."','".$application."','".$screening."','".$home_visit."','".$approval."', '".$followup."','".$clinic."','".$medical_services."','".$dogs_url."','".$cats_url."','".$_SERVER['HTTP_USER_AGENT']."','".$_SESSION['VFA_Userid']."', NOW());";
	//print_r($qry_update);
	//exit();
                  ...
                  if($result_upload >0)
	{
		...
		redirect_to('adopt_manage.php');;
	}
	else
	{
		$error="Updating failed!.";
		redirect_to('adopt_new.php');
	}
