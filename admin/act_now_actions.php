...

{
	if(move_uploaded_file($_FILES['image']['tmp_name'], $target_path. $_FILES["image"]['name'])) 
	{
	 ...
	 $qry_add="INSERT INTO `".TBL_ACT_NOW."` (`title`, `url`, `image`, `created_from`, `created_by`, `created_dt`) VALUES ('".$title."', '".$url."', '".$fileName."', '".$_SERVER['HTTP_USER_AGENT']."', '".$_SESSION['VFA_Userid']."', NOW());";
	$result_upload = $database->query( $qry_add );

	if($result_upload>0)
	{
		$msg="Updated successfully!.";
         redirect_to('act_now_manage.php?msg=Updated successfully!.');
	}
	else
	{
		$error="Updating failed!.";
       redirect_to('act_now_new.php?error=Updating failed!.');
	}
	}
	else 
	{
		redirect_to('act_now_new.php?error=all failed!.');
	}
        }

else
{
    redirect_to('act_now_new.php?error=File size is too large.');
}
}
