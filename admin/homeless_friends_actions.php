...
//}
$target_path = "images/homelessfriends/";

$mode=$_REQUEST["mode"];
$image_extensions_allowed = array('jpg', 'jpeg', 'png', 'gif','bmp');
if($mode=="homeless_friends")
{
	global $database, $db;
	...
	$image=$_REQUEST["image"];
        $_SESSION["homeless_friends_formData"]=$_REQUEST;
...

{
	if(move_uploaded_file($_FILES['image']['tmp_name'], $target_path. $_FILES["image"]['name'])) 
	{
	 ...
	 $qry_add="INSERT INTO `".TBL_HOMELESS_FRIENDS."` (`title`, `url`, `image`, `created_from`, `created_by`, `created_dt`) VALUES ('".$title."', '".$url."', '".$fileName."', '".$_SERVER['HTTP_USER_AGENT']."', '".$_SESSION['VFA_Userid']."', NOW());";
	$result_upload = $database->query( $qry_add );

	if($result_upload>0)
	{
		$msg="Updated successfully!.";
         redirect_to('homeless_friends_manage.php?msg=Updated successfully!.');
	}
	else
	{
		$error="Updating failed!.";
       redirect_to('homeless_friends_new.php?error=Updating failed!.');
	}
	}
	else 
	{
		redirect_to('homeless_friends_new.php?error=all failed!.');
	}
        }

else
{
    redirect_to('homeless_friends_new.php?error=File size is too large.');
}
}
